...
public class StarListFragment extends android.support.v4.app.Fragment implements View.OnClickListener{

    final private String childkey = Fire.KEY_ROOM_USERCOUNT;
    ...
    final private int LimitCount = 50;
...
//                addDataList(false);
    ...
    synchronized public void addDataList(final boolean replace){
        ...
        if(replace){
            mQuery = Fire.getReference().child(childkey).orderByChild("c").limitToLast(LimitCount);
        }
        else{
            mQuery = Fire.getReference().child(childkey).orderByChild("c").endAt(searchTime).limitToLast(LimitCount);
        }
                                ...
                                GlobalApplication.runOnMainThread(new Runnable() {
                                    ...
                                    public void run() {
                                        for(final FeedData f:list){
                                            Fire.getReference().child(Fire.KEY_CHAT_ROOM).child("0").child(f.getKey()).child("title").addListenerForSingleValueEvent(new ValueEventListener() {
                                                @Override
                                                public void onDataChange(DataSnapshot dataSnapshot) {
                                                    try {
                                                        if(dataSnapshot!=null && dataSnapshot.getValue()!=null) {
                                                            String title = (String)(dataSnapshot.getValue(true) + "");
                                                            f.setTitle(f.getuCount() +" / " + title + " || ");
                                                        }
                                                        else{
                                                            f.setTitle(f.getuCount() +" / null|||||||||");
                                                        }
                                                    }catch (Exception e){}
                                                    try {
                                                        synchronized (object) {
    ...
    public Comparator mComparatorNumber = new Comparator<FeedData>() {
        ...
        public int compare(FeedData s1, FeedData s2) {
            ...
            if(s1.getuCount() < s2.getuCount()){
                mReturn = 1;
            }
            else if(s1.getuCount() > s2.getuCount()){
                mReturn = -1;
            }
            else{
